...
function paintPageArea(pvo){
    ...
    if(pvo.startPage != 1){
        ...
        aTag.setAttribute("href" , `/approval/receiveList?pno=${pvo.startPage-1}`);
    ...
    for( let i = pvo.startPage ; i <= pvo.endPage ; i++ ){
        const aTag = document.createElement("a");
        const spanTag = document.createElement("span");
        aTag.setAttribute("href" , `/approval/receiveList?pno=${i}`);
        aTag.innerText = i;
        spanTag.appendChild(aTag);
        pageArea.appendChild(spanTag);
    }
    ...
    if(pvo.endPage != pvo.maxPage){
        ...
        aTag.setAttribute("href" , `/approval/receiveList?pno=${pvo.endPage+1}`);
        aTag.appendChild(iTag);
        spanTag.appendChild(aTag);
        pageArea.appendChild(spanTag);
    }
...
function receiveApprovalVoList(statusFilter){
    ...
    $.ajax({
        ...
        url : `/approval/receiveList/data?pno=${pno}` ,
        data : {
            statusFilter,
        },
       
        success : function(m){
            const receiveApprovalVoList = m.a;
            const pvo = m.b;
            paintPageArea(pvo);

            tbodyTag.innerHTML="";
            
            for(let vo of receiveApprovalVoList){
                ...
                tdTag04.innerText = vo.writerName;
...
function submitSearchForm(){
    ...
    receiveApprovalVoList(statusFilter);
